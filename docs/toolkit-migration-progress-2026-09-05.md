...
alchemy_tower, apartment, biofoundry, carriage_run, cultivation, daemon_directorate, dragons_den, dungeon_core, dungeon_manager, eclipse_heart, feast_frenzy, finallanding, frontier, hatchspire, idle_hands, iron_fauna, kaiju_sim, last_assembly, master_thief, mirexis, monsterhall, mytherra, nanite_swarm, occupational_hazard, planet_trader, planetfall_engineer, realmseed, sentience, stellar_legacy, tarrowyn, tb_realms, toybox, world_machine.

Completed: 5 of 38 active games. All five game repositories have clean working trees after staging every changed/untracked project file and committing. No outstanding compiler, test or publisher warnings in these games.

Next: cultivation or kaiju_sim JSON. Cultivation loader has four required native files, optional native files that fall back on read error but reject malformed JSON, balance/AI that also default on parse error, and WASM generic wrappers with lenient read/parse fallback. Preserve these policies explicitly in shared toolkit APIs; do not add another local generic loader. Its AGENTS.md matches canonical instructions (already compared). No cultivation files edited yet.
