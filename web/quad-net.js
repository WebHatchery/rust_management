// quad-net WASM bridge — the JS half of the `quad-net` crate's HTTP/WebSocket
// support on the web (miniquad target). It registers `http_make_request` /
// `http_try_recv` (and the `ws_*` WebSocket functions) as miniquad env imports,
// so a macroquad game built against `quad-net` can talk to a server from the
// browser. Inert for games that never call it — like the clipboard bridge, it
// only *adds* functions to the import object.
//
// Provenance: verbatim from quad-net 0.1.2 (`js/quad-net.js`). Depends on the
// globals from mq_js_bundle.js (`miniquad_add_plugin`) and sapp_jsutils.js
// (`consume_js_object` / `js_object`), so it must load AFTER both and BEFORE the
// wasm `load()` call (miniquad wires registered plugins into the import object
// at instantiation). Update this file if the crate version bumps.
